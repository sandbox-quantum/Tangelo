...
def translate_qulacs(source_circuit, noise_model=None):
    ...
    for gate in source_circuit._gates:
        if gate.name in {"H", "X", "Y", "Z", "S", "T"}:
            ...
            mat_gate = qulacs.gate.to_matrix_gate(GATE_QULACS[gate.name](gate.target))
            mat_gate.add_control_qubit(gate.control, 1)
            ...
            mat_gate = qulacs.gate.to_matrix_gate(GATE_QULACS[gate.name](gate.target, -1. * gate.parameter))
            mat_gate.add_control_qubit(gate.control, 1)
            target_circuit.add_gate(mat_gate)
        elif gate.name in {"SWAP"}:
            (GATE_QULACS[gate.name])(target_circuit, gate.target, gate.target1)
        elif gate.name in {"CSWAP"}:
            mat_gate = qulacs.gate.to_matrix_gate(GATE_QULACS[gate.name](gate.target, gate.target1))
            mat_gate.add_control_qubit(gate.control, 1)
            target_circuit.add_gate(mat_gate)
        elif gate.name in {"PHASE"}:
            mat_gate = GATE_QULACS[gate.name](gate.target, [[1, 0], [0, exp(1j * gate.parameter)]])
            target_circuit.add_gate(mat_gate)
