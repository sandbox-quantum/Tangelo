...
def hermitian_conjugate(terms):
    ...
    """
    try:
        return [[((v[0][3][0],1),(v[0][2][0],1),(v[0][1][0],0),(v[0][0][0],0)),-v[1]] for v in terms]
    except:
...
def get_spin_ordered(n_orbs,pp,qq,rr=-1,ss=-1,up_down = False):
    r"""For a given set of orbitals, provide corresponding spin-orbital indices,
...
def get_group_1_2(n_orbs,p,q,r,s,up_down=False):
    ...
    triplet = triplet + hermitian_conjugate(triplet)
    
    singlet = singlet + hermitian_conjugate(singlet)
...
def get_all_excitations(n_orbs,up_down = False):
    r"""Enumerate all possible single and double excitations
...
def uccgsd_generator(n_qubits,single_coeffs=None,double_coeffs=None,up_down=False):
    r"""Construct a list of FermionOperators enumerating the Unitary Coupled-Cluster
...
def factorial(n):
    r"""Simple recursive helper function for evaluating factorial.

    Args:
        n (int or float): number, must be int, or integer-valued float

    Returns:
        int or float equal to \prod_i=1^n i
    """
    if not (type(n)==float or type(n)==int):
        raise TypeError("Factorial requires numeric-type input.")
    if n!= np.floor(n):
        raise TypeError("Factorial requires integer input.")
    if n<0:
        raise ValueError("Factorial not defined for negative input.")
...
if __name__ == "__main__":
    ...
    fermions = uccgsd_generator(8)
    nqbits = np.linspace(2,35,10,dtype=int)*2
    nterms = np.zeros(len(nqbits))
    import matplotlib.pyplot as plt
    for ni in range(len(nqbits)):
        print(nqbits[ni])
        nterms[ni] = len(uccgsd_generator(int(nqbits[ni])))
    
    fig,ax=plt.subplots(1,1)
    ax.plot(nqbits,nterms,'-o')
    ax.set_xlabel('Number of Qubits')
    ax.set_ylabel('Number of Pool Elements')
    ax.set_yscale('log')
    plt.show()