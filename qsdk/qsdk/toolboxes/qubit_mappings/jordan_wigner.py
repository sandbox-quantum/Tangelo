...
def jordan_wigner(operator):
    ...
    raise TypeError("Operator must be a FermionOperator,MajoranaOperator, "
                    "DiagonalCoulombHamiltonian, or InteractionOperator.")
...
def jordan_wigner_interaction_op(iop, n_qubits=None):
    ...
    One could accomplish this very easily by first mapping to fermions and then
    mapping to qubits. We skip the middle step for the sake of speed.
...
def jordan_wigner_two_body(p, q, r, s, coefficient=1.):
    ...
    Note that the diagonal terms are divided by a factor of two because they are
    equal to their own Hermitian conjugate.
    """
