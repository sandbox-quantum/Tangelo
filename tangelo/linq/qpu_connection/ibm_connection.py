...
class IBMConnection(QpuConnection):
    ...
    def __init__(self):
        self.endpoint = "https://us-east.quantum-computing.cloud.ibm.com/"
        self.api_key = None
    ...
    def job_submit(self, target_backend, abs_circuit, n_shots, job_name, **job_specs):
        ...
        ibm_circuit = translate_qiskit(abs_circuit)

        with Sampler(circuits=ibm_circuit, service=self.service, options={"backend": "ibmq_qasm_simulator"}) as sampler:
            result = sampler(circuits=[0], shots=n_shots)
            print(result)

        print()
        #self._catch_request_error()
