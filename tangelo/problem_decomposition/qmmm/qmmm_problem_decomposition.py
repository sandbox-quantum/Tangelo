...
class QMMMProblemDecomposition(ProblemDecomposition):
    ...
    def __init__(self, opt_dict: dict):
        ...
        if self.charges is not None and len(self.charges) != len(self.coords):
        ...
        if self.charges is None and self.geometry[-3:] == "pdb":
            ...
            _ = conv.ReadFile(mol, self.pdbfile)
            self.geometry = conv.WriteString(mol)
            self.geometry = self.geometry.split("\n", 2)[2:][0]
    ...
    def distribute_atoms(self):
        ...
        # Case where an int is detected -> first n atoms.
            ...
            if self.coords is None:
                ...
                self.mmcharges = self.charges[self.qmfragment.selected_atoms:]
        ...
        # First atom is 0.
            ...
            if self.coords is None:
                ...
                self.mmcharges = [self.charges[n] for n in range(len(self.geometry)) if n not in self.qmfragment.selected_atoms]
